...
public class BookingModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        try
        {
            ...
            // Enforce rule: only one active room booking per customer
            // A booking is considered active if it is confirmed and the check-out date is in the future or today
            var hasActiveRoomBooking = await _context.Bookings.AnyAsync(b =>
                b.CustomerId == userId.Value &&
                b.BookingStatus == "confirmed" &&
                b.CheckOutDate.Date >= DateTime.Today);

            if (hasActiveRoomBooking)
            {
                _logger.LogWarning("Booking failed: Customer {CustomerId} already has an active room booking", userId.Value);
                ErrorMessage = "You already have an active room booking. Please complete or cancel your current stay before creating a new room booking.";
                Currency = await _currencyHelper.GetCurrencyAsync();
                await LoadRoomDataAsync();
                return Page();
            }
    ...
    public async Task<IActionResult> OnPostTravelBookingAsync()
    {
        ...
        // Enforce rule: only one active travel booking per customer per travel date
        var hasExistingTravelBookingSameDay = await _context.TravelBookings.AnyAsync(t =>
            t.CustomerId == userId.Value &&
            t.BookingStatus == "confirmed" &&
            t.TravelDate.Date == TravelInput.TravelDate.Date);

        if (hasExistingTravelBookingSameDay)
        {
            ErrorMessage = "You already have a travel booking on this date. Please choose a different date or modify your existing travel booking.";
            Attraction = TravelInput.AttractionName;
            Type = TravelInput.AttractionType;
            return Page();
        }
